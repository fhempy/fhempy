...
class warema(FhemModule):
    def __init__(self, logger):
        ...
        attr_config = {
            "interval": {"default": 60, "format": "int", "help": "Change interval, default is 60."},
        }
    ...
    async def Define(self, hash, args, argsh):
        ...
        self.hash = hash
        if len(args) < 5:
            return "Usage: define warema_fhempy PythonModule warema <IP> <channel>"
        ...
        if await fhem.AttrVal(self.hash["NAME"], "devStateIcon", "") == "":
            ...
            await fhem.CommandAttr(self.hash, self.hash["NAME"] + " devStateIcon " + devStateIcon)
        if await fhem.AttrVal(self.hash["NAME"], "webCmd", "") == "":
            await fhem.CommandAttr(self.hash, self.hash["NAME"] + " webCmd up:down:status")
        if await fhem.AttrVal(self.hash["NAME"], "stateFormat", "") == "":
            await fhem.CommandAttr(self.hash, self.hash["NAME"] + " stateFormat position")
    ...
    async def set_attr_interval(self, hash):
        await fhem.readingsSingleUpdate(self.hash, "interval", str(self._attr_interval), 1)
        self.updateTask = self.create_async_task(self.update_task())
    ...
    async def set_up(self, hash, params):
        self._warema_shades[self._warema_channel].set_shade_position(0) #0=open; 100=closed
        state = self._warema_shades[self._warema_channel].get_shade_state(True) #Force update and get shade state
        (position,ismoving,date) = state
    ...
    async def set_down(self, hash, params):
        self._warema_shades[self._warema_channel].set_shade_position(100) #0=open; 100=closed
        state = self._warema_shades[self._warema_channel].get_shade_state(True) #Force update and get shade state
        (position,ismoving,date) = state
    ...
    async def set_position(self, hash, params):
        ...
        self._warema_shades[self._warema_channel].set_shade_position(pos) #0=open; 100=closed
        state = self._warema_shades[self._warema_channel].get_shade_state(True) #Force update and get shade state
        (position,ismoving,date) = state
